    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.modulation = ParallelExponentialModulation(
            neox_args, d_model, self.hidden_size_per_partition, mpu.get_model_parallel_rank(), **kwargs)
...
class ParallelH3Conv(nn.Module):
    ...
    def __init__(
        ...
        _initialize_affine_weight_gpu(self.short_conv_weight, init_method, partition_dim=1)
        ...
        if not self.use_hyena_filter:
            ...
            _initialize_affine_weight_gpu(self.long_conv_weight, init_method, partition_dim=0)
        else:
...
class ParallelHyenaConv(nn.Module):
    ...
    def forward(self, query_layer, key_layer, value_layer):
        ...
        with torch.autocast("cuda"):
            z = fftconv_ref(z.to(torch.float32), filter.to(torch.float32), self.long_conv_bias, None, gelu=False)
            z = z.to(v.dtype)
        z = z * self.act(q)
