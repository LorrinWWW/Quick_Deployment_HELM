...
def log_norm(norm, key, iteration_no):
    if norm is not None:
        tb_wandb_log(key, norm, iteration_no, use_wandb=True, tensorboard_writer=None, all_ranks=False)
...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.use_hyena = self.attention_type == "hyena" or self.attention_type == "hyena_v2"
        self.use_flash_attention = self.attention_type == "flash"
        self.sparse = self.attention_type not in ("global", "flash", "h3", "badger", "hyena", "hyena_v2")
        ...
        if self.sparse:
            ...
            if self.use_h3:
                from megatron.model.long_conv import ParallelH3Conv
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None):
        ...
        if self.attention_type == "hyena_v2":
            ...
            if self.layer_number in [0,1]:
                print(self.layer_number, output[:2, 0])
