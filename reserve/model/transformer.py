...
from reserve.model.fused_softmax import FusedScaleMaskSoftmax
from reserve.model.activations import get_activation
from reserve.model.utils import exists, get_fusion_type
from reserve.model.positional_embeddings import (
    RotaryEmbedding,
    apply_rotary_pos_emb,
    apply_rotary_pos_emb_torch,
    AliBi,
)
from reserve.model.fused_bias_dropout import (
    get_bias_dropout_add,
    bias_dropout_add_fused_train,
    bias_dropout_add_fused_inference,
)
from reserve.model.utils import configure_sparse_attention
...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.dtype = get_dtype_from_string(neox_args.precision)
        ...
        self.use_h3 = self.attention_type == "h3"
        self.use_badger = self.attention_type == "badger"
        self.use_hyena = (
            self.attention_type == "hyena" or self.attention_type == "hyena_v2"
        )
        ...
        if self.sparse:
            ...
            if self.use_h3:
                from reserve.model.long_conv import ParallelH3Conv

                self.mixer = ParallelH3Conv(neox_args)
            elif self.use_badger:
                from reserve.model.long_conv import ParallelBadgerConv

                self.mixer = ParallelBadgerConv(neox_args)
            elif self.use_hyena:
                if self.attention_type == "hyena":
                    from reserve.model.hyena import ParallelHyenaConv

                    self.hyena_proj_conv = nn.Conv1d(
                        3 * self.hidden_size_per_partition,
                        3 * self.hidden_size_per_partition,
                        neox_args.short_conv_L,
                        groups=3 * self.hidden_size_per_partition,
                        padding=neox_args.short_conv_L - 1,
                    )

                    self.mixer = ParallelHyenaConv(neox_args, init_method, layer_number)
                elif self.attention_type == "hyena_v2":
                    from reserve.model.hyena import ParallelHyenaConv2
                ...
                from reserve.model.flash_attention import (
