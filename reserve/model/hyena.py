...
from reserve import mpu
from reserve.ops.fftconv import fftconv_func, _mul_sum
from reserve.model.init_functions import xavier_normal_init_method
from reserve.logging import tb_wandb_log
from reserve.model.utils import get_dtype_from_string
        ...
        **kwargs,
        ...
        if neox_args.hyena_pos_emb == "fourier_fixed":
            ...
            self.pos_emb = PositionalEmbedding(
                emb_dim, seq_len, dtype=neox_args.precision
            )
        ...
        fast_decay_pct, slow_decay_pct = (
            neox_args.hyena_filter_fast_decay,
            neox_args.hyena_filter_slow_decay,
        )
    ...
    def __init__(
        ...
        if neox_args.hyena_filter_cls == "implicit":
            self.filter = ParallelHyenaFilter(
                ...
                normalized=neox_args.normalize_hyena_filters,
...
class DoubleGateParallelHyenaConv(ParallelHyenaConv):
    """
    Inner action for a parallel Hyena Conv (replacing attention).
    Testing some new improvements...
    ...
    def __init__(
        ...
        super().__init__(neox_args, init_method, layer_number, smaller_hidden_size)

    def forward(self, value_layer, key_layer0, query_layer0, key_layer1, query_layer1):
        # input sizes: [sq, b, np, hn]
        # seqlen, batch, tensor parallel, hidden size per tensor parallel
        np = value_layer.shape[-2]
        L = value_layer.shape[0]

        value = rearrange(value_layer, "sq b np hn -> b (np hn) sq")
        key0 = rearrange(key_layer0, "sq b np hn -> b (np hn) sq")
        query0 = rearrange(query_layer0, "sq b np hn -> b (np hn) sq")
        key1 = rearrange(key_layer1, "sq b np hn -> b (np hn) sq")
        query1 = rearrange(query_layer1, "sq b np hn -> b (np hn) sq")
        ...
        # check if hidden size different
        if int(value.shape[1]) != self.hidden_size:
            input_projection_size = value.shape[1]
            head_dim = input_projection_size // self.num_heads
            h = h.repeat_interleave(head_dim, dim=0)
        else:
            h = h.repeat_interleave(self.head_dim, dim=0)

        # double gate
        z = self.act(v * self.act(k0) * self.act(q0))
        ...
        # double gate
        z = self.act(z * self.act(k1) * self.act(q1))

        return rearrange(z, "b (np hn) sq -> b np sq hn", np=np)


class DoubleGateParallelHyenaConv2(nn.Module):
    """
    Double gate hyena, but the projections can be smaller than the hidden dimension.
    ...
    def __init__(
        ...
        world_size = mpu.get_model_parallel_world_size()
        self.hidden_size_per_partition = mpu.divide(new_hidden_size, world_size)
